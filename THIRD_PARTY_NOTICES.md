...
本项目（PC Control Center）采用 **GNU General Public License v3.0 (GPL-3.0-or-later)** 开源许可证。完整许可证文本参见仓库根目录 [LICENSE](LICENSE)。
...
- **实现差异与独立性**：
  ...
  3. 引入了硬件型号白名单核验、双向进程令牌核验、运行期间显式授权、心跳租约与恢复机制，以减少误用风险。自动化验证不能代替各机型实测，也不能保证异常断电或强制终止整个进程树时恢复成功。
...
- **Microsoft .NET 10 Runtime**：采用 MIT License。自包含（Self-Contained）发布包内包含官方运行库，并附带官方发布的 `DOTNET-LICENSE.txt` 与 `DOTNET-THIRD-PARTY-NOTICES.txt`；这两个文件在解压后的发布目录中。
