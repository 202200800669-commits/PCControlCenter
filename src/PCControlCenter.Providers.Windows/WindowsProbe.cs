...
public enum ProbeKind { Identity, Generic, ThinkBookFans, ThinkBookMode }
...
public sealed class WindowsProbe : IReadOnlyProbe {
 ...
 public async Task<JsonElement> QueryAsync(ProbeKind kind,CancellationToken ct) {
  ...
  var script=kind switch {ProbeKind.Identity=>IdentityScript,ProbeKind.Generic=>GenericScript,ProbeKind.ThinkBookFans=>FanScript,ProbeKind.ThinkBookMode=>ModeScript,_=>throw new ArgumentOutOfRangeException(nameof(kind))};
  ...
  try {
   ...
   if(kind==ProbeKind.Generic) {
    ...
    node["displays"]=InstalledDisplayDrivers();text=node.ToJsonString();
   }
 ...
 private static JsonArray InstalledDisplayDrivers() {
  ...
  try {
   ...
   if(drivers is null)return result;
   foreach(var name in drivers.GetSubKeyNames().Where(n=>n.Length==4&&n.All(char.IsDigit)).Order()) {
    using var key=drivers.OpenSubKey(name);if(key is null)continue;
    ...
    if(description.Length==0||version.Length==0)continue;
