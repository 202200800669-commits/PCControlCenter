...
public sealed record BrokerRequest(int Version, string RequestId, string Operation, DeviceIdentity Device, FanTrial? Trial = null, ModeRequest? Mode = null);
public sealed record BrokerResponse(int Version, string RequestId, string Code, int? Fan1 = null, int? Fan2 = null, FanReceipt? Receipt = null, ModeReceipt? ModeReceipt = null);
public static class BrokerProtocol
{
    ...
    public static bool Valid(BrokerRequest q) => q.Version == Version && Guid.TryParseExact(q.RequestId, "N", out _) && ValidIdentity(q.Device) && ((q.Operation == "read-fans" && q.Trial is null && q.Mode is null) || (q.Operation == "fan-trial" && q.Trial is { IsValid: true } && q.Mode is null) || (q.Operation == "set-mode" && q.Mode is { IsValid: true } && q.Trial is null));
