...
public sealed class ElevatedProbe(IReadOnlyProbe normal, DeviceIdentity device) : IReadOnlyProbe
{
    ...
    private async Task<JsonElement> ReadFansAsync(CancellationToken ct)
    {
        ...
        if (response.Code != "OK")
            throw new ProbeException(response.Code);
        if (response.Fan1 is null or < 0 or > 10000 || response.Fan2 is null or < 0 or > 10000)
            throw new ProbeException("BROKER_INVALID_READING");
