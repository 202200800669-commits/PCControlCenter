...
public static class BrokerClient {
 public static async Task<BrokerResponse> ExecuteAsync(BrokerRequest request,CancellationToken ct) {
  if(!BrokerProtocol.Valid(request))throw new ArgumentException("INVALID_REQUEST");
  ...
  using var timeout=CancellationTokenSource.CreateLinkedTokenSource(ct);timeout.CancelAfter(TimeSpan.FromSeconds(100));
  ...
  try{response=await BrokerProtocol.ReceiveAsync<BrokerResponse>(pipe,timeout.Token);}
  ...
  if(response.Version!=BrokerProtocol.Version||response.RequestId!=request.RequestId)throw new ProbeException("BROKER_PROTOCOL_MISMATCH");
  return response;
 }
}
