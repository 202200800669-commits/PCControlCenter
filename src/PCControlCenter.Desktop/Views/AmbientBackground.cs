    ...
    public void Rebuild()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            var glow = new Ellipse
            {
                ...
                Fill = fill,
                CacheMode = new BitmapCache { RenderAtScale = .65 },
            ...
            var opacity = new DoubleAnimation(.65, 1, TimeSpan.FromSeconds(6 + i * 1.9))
            {
                AutoReverse = true,
                RepeatBehavior = RepeatBehavior.Forever
            };
            Timeline.SetDesiredFrameRate(opacity, 30);
            glow.BeginAnimation(OpacityProperty, opacity);
        }
    }
