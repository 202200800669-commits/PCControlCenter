...
public static class Feedback
{
    ...
    public static FeedbackSummary Inspect(string file)
    {
        ...
        if (input.Length > MaxZip)
            throw new InvalidDataException("ARCHIVE_TOO_LARGE");
        using var zip = new ZipArchive(input, ZipArchiveMode.Read);
        if (zip.Entries.Count != 1 || zip.Entries[0].FullName != "diagnostics.json")
            throw new InvalidDataException("UNEXPECTED_ENTRY");
        var entry = zip.Entries[0];
        if (entry.Length > MaxJson)
            throw new InvalidDataException("REPORT_TOO_LARGE");
        ...
        if (root.ValueKind != JsonValueKind.Object)
            throw new InvalidDataException("INVALID_ROOT");
        UniqueKeys(root);
        int version = root.GetProperty("schemaVersion").GetInt32();
        if (version is not (1 or 2))
            throw new InvalidDataException("UNSUPPORTED_SCHEMA");
        var device = root.GetProperty("device");
        if (device.ValueKind != JsonValueKind.Object)
            throw new InvalidDataException("INVALID_DEVICE");
        string Field(JsonElement value, string name)
        {
            var field = value.GetProperty(name);
            if (field.ValueKind != JsonValueKind.String)
                throw new InvalidDataException("INVALID_FIELD");
            var text = field.GetString() ?? "";
            if (text.Length > 160)
                throw new InvalidDataException("FIELD_TOO_LONG");
            return PublicText.Clean(text);
        }
        ...
        if (identity.Manufacturer.Length == 0 || identity.Product.Length == 0 || identity.Bios.Length == 0)
            throw new InvalidDataException("INCOMPLETE_IDENTITY");
        var features = root.GetProperty("capabilities");
        if (features.ValueKind != JsonValueKind.Array || features.GetArrayLength() > 64)
            throw new InvalidDataException("INVALID_CAPABILITIES");
        ...
        if (version == 2 && root.TryGetProperty("system", out var details) && details.ValueKind != JsonValueKind.Null)
        {
            if (details.ValueKind != JsonValueKind.Object)
                throw new InvalidDataException("INVALID_SYSTEM");
    ...
    private static void UniqueKeys(JsonElement e)
    {
        if (e.ValueKind == JsonValueKind.Object)
        {
            ...
            {
                if (!names.Add(p.Name))
                    throw new InvalidDataException("DUPLICATE_KEY");
                UniqueKeys(p.Value);
            }
        }
