...
public static class PreferenceStore {
 ...
 public static PreferenceState Read(string path) {
  ...
  if(input.Length>16384)throw new InvalidDataException("Configuration too large");
  using var data=new MemoryStream();input.CopyTo(data);var bytes=data.ToArray();
  using var document=JsonDocument.Parse(bytes,new JsonDocumentOptions{MaxDepth=4});
  if(document.RootElement.ValueKind!=JsonValueKind.Object)throw new InvalidDataException("Invalid configuration");
  var names=new HashSet<string>(StringComparer.Ordinal);
  foreach(var field in document.RootElement.EnumerateObject()) {
   if(field.Name is not ("SchemaVersion" or "MinimizeToTray" or "PollIntervalSeconds")||!names.Add(field.Name))throw new InvalidDataException("Unknown or duplicate setting");
  }
  if(names.Count!=3)throw new InvalidDataException("Incomplete configuration");
  var p=JsonSerializer.Deserialize<Preferences>(bytes)??throw new InvalidDataException("Empty configuration");
  Validate(p);return new(p,Convert.ToHexString(SHA256.HashData(bytes)));
 }
 public static PreferenceUpdate Update(string path,string expectedRevision,Preferences preferences) {
  ...
  if(Read(full).Revision!=expectedRevision)throw new InvalidOperationException("CONFIGURATION_CONFLICT");
  ...
  try {
   using(var output=new FileStream(temporary,FileMode.CreateNew,FileAccess.Write,FileShare.None)) {
    JsonSerializer.Serialize(output,preferences,Diagnostics.Json);output.Flush(true);
   }
