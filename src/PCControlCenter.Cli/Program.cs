...
if (args is ["feedback-template"] || args is ["feedback-template", var destMd])
{
    try
    {
        ...
        var device = await WindowsProbe.IdentifyAsync(probe, cts.Token);
        var registry = Providers.Create(probe);
        var provider = registry.Resolve(device);
        var snap = await provider.ReadAsync(device, cts.Token);
    ...
    {
        Console.Error.WriteLine($"生成反馈模板失败: {ex.Message}");
        return 13;
    }
}
