...
try {
 ...
 try {
  ...
  if(actual!=request.Device||!new ThinkBookProvider(probe).Matches(actual))response=new(BrokerProtocol.Version,request.RequestId,"IDENTITY_MISMATCH");
  else {
   if(request.Operation=="read-fans") {
    ...
    async Task WatchDisconnect() {
     try{var buffer=new byte[1];var count=await pipe.ReadAsync(buffer,watch.Token);if(count is 0 or 1)stop.Cancel();}
     catch(OperationCanceledException){}catch(IOException){stop.Cancel();}
    }
    var disconnected=WatchDisconnect();
    var receipt=await FanSessionRunner.RunAsync(request.Trial!,stop.Token);
    watch.Cancel();await disconnected;
