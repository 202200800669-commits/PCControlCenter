...
> 当前为 **0.1.0-alpha.18 社区试点版**。多品牌框架已经建立，但不能据此理解为所有品牌都能调风扇。华硕、机械革命等设备目前主要用于品牌识别、只读检测和反馈采集；专用硬件写入仍按精确型号与 BIOS 白名单开放。

alpha.18 新增手动转速定时：在“性能与散热 → 手动转速 → 运行时长”选择持续运行或 1、5、10、15、30、60、120 分钟。松开转速滑块开始运行，到期恢复自动散热。运行中调整转速不重置倒计时；更换时长从选择时重新计时，切回持续运行取消定时。最小化不影响定时。

alpha.17 的后台读取、可见页面合并刷新和缓存动效优化继续保留。[性能测试与范围](docs/performance.md)
