...
static class FanWorkerTests {
 ...
 private static async Task<(JsonElement Receipt,JsonElement Writes)> RunAsync(Scenario s,string? mutexName=null) {
  ...
  foreach(var arg in new[]{"-NoProfile","-NonInteractive","-EncodedCommand",Convert.ToBase64String(Encoding.Unicode.GetBytes(body))})start.ArgumentList.Add(arg);
  ...
  try {
   if(s.Disconnect) {
    ...
    if(ready!="READY")throw new Exception("Worker did not enter simulated write");
    process.StandardInput.Close();
   }
   var output=process.StandardOutput.ReadToEndAsync();
   await process.WaitForExitAsync().WaitAsync(TimeSpan.FromSeconds(15));
   if(process.ExitCode!=0)throw new Exception("Mock worker failed: "+await errors);
   var lines=(await output).Split(['\r','\n'],StringSplitOptions.RemoveEmptyEntries);
   if(lines.Length!=2)throw new Exception("Unexpected mock output: "+string.Join(" / ",lines));
