...
$entries=@(Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json | ForEach-Object { $_ })
if($entries.Count -eq 0 -or $entries.Count -gt 1024){throw 'Invalid manifest size'}
$names=New-Object 'System.Collections.Generic.HashSet[string]' ([StringComparer]::OrdinalIgnoreCase)
foreach($entry in $entries) {
 if($entry.file -isnot [string] -or $entry.file -notmatch '^[A-Za-z0-9][A-Za-z0-9._-]{0,200}$' -or $entry.file -eq 'SHA256.json' -or !$names.Add($entry.file)){throw ('Invalid or duplicate manifest name: '+$entry.file)}
...
foreach($required in @('pc-control.exe','pc-control-broker.exe','pc-control-desktop.exe','coreclr.dll','BUILD.json','DOTNET-LICENSE.txt','DOTNET-THIRD-PARTY-NOTICES.txt','LICENSE')) {
